...
"""
    specialize(f::RE, α) where {RE <: AA.MPolyRingElem}
...
```
"""
function specialize(f::RE, α) where {RE <: AA.MPolyRingElem}
    AA.map_coefficients(c -> evaluate(c, α), f)
end


"""
    CGS_simple(F::Vector{RE}) where {RE <: AbstractAlgebra.MPolyRingElem}

Simple function to compute Gröbner systems. Returns a vector of triples of the form `(S, [h], G)`.
For any vector `α` such that `specialize.(S, Ref(α)) == [0, 0, ..., 0]` and `specialize(h, α) != 0`,
`specialize.(G, Ref(α))` is a Gröbner basis.
...
"""
    CGB(F::Vector{RE}, reduced=true) where {RE<:AbstractAlgebra.MPolyRingElem}

Computes a comprehensive parametric Gröbner basis of the ideal generated by `F`. In other words
`specialize.(G, Ref(α))` is a Gröbner basis for any vector `α` of appropriate length.

If the option `reduced=true`, then `specialize.(G, Ref(α))` always contains the reduced Gröbner basis of
the ideal generated by `specialize.(F, Ref(α))`.
"""
function CGB(F::Vector{RE}, reduced=true) where {RE<:AA.MPolyRingElem}
    # S = [AA.leading_coefficient(g) for g ∈ groebner_wrapper(F) if AA.is_constant(g)]
    # G = CGBMain(F, S, red)
    A = AA.base_ring(AA.parent(F[1]))
    G = CGBMain(F, empty([one(A)]), reduced)
    return unique(vcat([G_ for (_, _, G_) ∈ G]...))
end

"""
    CGS_faithful(F::Vector{RE}, reduced=true) where {RE<:AbstractAlgebra.MPolyRingElem}

Function to compute faithful, reduced Gröbner systems. Returns a vector of triples of the form `(S, [h], G)`.
For any vector `α` such that `specialize.(S, α) == [0, 0, ..., 0]` and `specialize(h, α) != 0`,
`specialize.(G, Ref(α))` is a reduced Gröbner basis. Furthermore, each `g ∈ G` will belong to the ideal
generated by `F`.

Takes the argument `reduced`. If set to false, the returned Gröbner basis will not be reduced.

See also [`CGS_simple`](@ref)
"""
